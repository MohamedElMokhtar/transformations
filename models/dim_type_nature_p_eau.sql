...
-- Step 1: Collect all type_nature_p_eau from all RAW source tables
with all_type_nature_p_eau as (
  select
     id as nature_p_eau_id,
  designation_latin as nature_p_eau,
    'AHS' as src,
  ...
  from {{ source('public', 'ahs_type_nature_p_eau') }}                                       -- ######################## To be changed
  ...
  from {{ source('public', 'csm_type_nature_p_eau') }}                                       -- ######################## To be changed
  ...
  designation_latin as nature_p_eau,
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'occ_type_nature_p_eau') }}                                       -- ######################## To be changed
  ...
  designation_latin as nature_p_eau,
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'cz_type_nature_p_eau') }}                                       -- ######################## To be changed
  ...
  designation_latin as nature_p_eau,
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'sahara_type_nature_p_eau') }}                                       -- ######################## To be changed
),

-- Step 2: Prepare source data with natural key (src_id) and apply incremental filter
source_data as (
  select
    src || '_' || nature_p_eau_id as src_id,
     nature_p_eau_id,
  nature_p_eau,
    src,
    _ab_cdc_updated_at,
    _ab_cdc_deleted_at,
    row_number() over (partition by src || '_' || nature_p_eau_id order by _ab_cdc_updated_at desc) as rn  -- ADDED
  from all_type_nature_p_eau
  {% if is_incremental() %}
...
, changed_records as (
  ...
  where
    ...
    (s.nature_p_eau is distinct from t.nature_p_eau)                              -- ######################## To be changed THIS PART you put only the data (don't put id)
            -- ######################## To be hanged
)
