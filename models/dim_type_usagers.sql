...
-- Step 1: Collect all type_unite from all RAW source tables
with all_type_unite as (
  select
     id as type_usagers_id,
  designation_latin as type_usagers,
Taux_Budjet_Etat,
  Taux_Fne,
  Taux_agire,
    'AHS' as src,
  ...
  from {{ source('public', 'ahs_type_unite') }}                                       -- ######################## To be changed
...
Taux_Budjet_Etat,
  ...
  from {{ source('public', 'csm_type_unite') }}                                       -- ######################## To be changed
...
Taux_Budjet_Etat,
  ...
  Taux_agire,
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'occ_type_unite') }}                                       -- ######################## To be changed
...
Taux_Budjet_Etat,
  ...
  Taux_agire,
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'cz_type_unite') }}                                       -- ######################## To be changed
...
Taux_Budjet_Etat,
  ...
  Taux_agire,
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'sahara_type_unite') }}                                       -- ######################## To be changed
),

-- Step 2: Prepare source data with natural key (src_id) and apply incremental filter
source_data as (
  select
    src || '_' || type_usagers_id as src_id,
      type_usagers_id,
  type_usagers,
Taux_Budjet_Etat,
  Taux_Fne,
  Taux_agire,
    ...
    _ab_cdc_updated_at,
    _ab_cdc_deleted_at,
    row_number() over (partition by src || '_' || type_usagers_id order by _ab_cdc_updated_at desc) as rn  -- ADDED
  from all_type_unite
  {% if is_incremental() %}
...
s.Taux_Budjet_Etat,
  ...
  where
    ...
    (s.type_usagers is distinct from t.type_usagers)                              -- ######################## To be changed THIS PART you put only the data (don't put id)
           -- ######################## To be changed
)
...
t.Taux_Budjet_Etat,
  ...
  from {{ this }} t
  inner join all_type_unite s
    on t.src_id = (s.src || '_' || s.type_usagers_id)
  where t.is_current = true
