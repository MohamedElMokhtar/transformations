...
-- Step 1: Collect all recours from all RAW source tables
with all_type_activite as (
  ...
  from {{ source('public', 'ahs_recours') }}                                       -- ######################## To be changed
  ...
  from {{ source('public', 'csm_recours') }}                                       -- ######################## To be changed
  ...
  date_recour,
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'occ_recours') }}                                       -- ######################## To be changed
  ...
  date_recour,
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'cz_recours') }}                                       -- ######################## To be changed
...
date_recour,
    ...
    _ab_cdc_updated_at,
    _ab_cdc_deleted_at,
    row_number() over (partition by src || '_' || recours_id order by _ab_cdc_updated_at desc) as rn  -- ADDED
...
t.date_recour,
  ...
  inner join all_type_activite s
    on t.src_id = (s.src || '_' || s.recours_id)
  where t.is_current = true
