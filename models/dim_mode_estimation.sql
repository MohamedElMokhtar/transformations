...
-- Step 1: Collect all type_estimation from all RAW source tables
with all_type_estimation as (
  select
    id as mode_estimation_id,
designation_latin as mode_estimation,
  ...
  from {{ source('public', 'ahs_type_estimation') }}                                       -- ######################## To be changed
...
designation_latin as mode_estimation,
  ...
  from {{ source('public', 'csm_type_estimation') }}                                       -- ######################## To be changed
...
designation_latin as mode_estimation,
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'occ_type_estimation') }}                                       -- ######################## To be changed
...
designation_latin as mode_estimation,
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'cz_type_estimation') }}                                       -- ######################## To be changed
...
designation_latin as mode_estimation,
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'sahara_type_estimation') }}                                       -- ######################## To be changed
),
...
mode_estimation,
    ...
    _ab_cdc_updated_at,
    _ab_cdc_deleted_at,
    row_number() over (partition by src || '_' || mode_estimation_id order by _ab_cdc_updated_at desc) as rn  -- ADDED
  from all_type_estimation
  {% if is_incremental() %}
...
s.src,
  ...
  where
    ...
    (s.mode_estimation is distinct from t.mode_estimation)                              -- ######################## To be changed THIS PART you put only the data (don't put id)
             -- ######################## To be changed
)
...
t.mode_estimation,
  ...
  from {{ this }} t
  inner join all_type_estimation s
    on t.src_id = (s.src || '_' || s.mode_estimation_id)
  where t.is_current = true
