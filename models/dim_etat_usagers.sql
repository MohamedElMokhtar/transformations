...
with all_etat_usagers as (
  select
    id as etat_usagers_id,
    designation_latin as etat_usager,
    'AHS' as src,
  ...
  from {{ source('__raw_', 'ahs_type_etat_unite') }}
  ...
  from {{ source('__raw_', 'csm_type_etat_unite') }}
  ...
  select
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('__raw_', 'occ_type_etat_unite') }}
  ...
  select
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('__raw_', 'cz_type_etat_unite') }}
  ...
  select
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('__raw_', 'sahara_type_etat_unite') }}
),

-- Step 2: Prepare source data with natural key (src_id) and apply incremental filter
source_data as (
  select
    src || '_' || etat_usagers_id as src_id,
    etat_usagers_id,
    etat_usager,
    src,
    _ab_cdc_updated_at,
    _ab_cdc_deleted_at,
    row_number() over (partition by src || '_' || etat_usagers_id order by _ab_cdc_updated_at desc) as rn  -- ADDED
  from all_etat_usagers
  {% if is_incremental() %}
...
, changed_records as (
  ...
  where
    ...
    (s.etat_usager is distinct from t.etat_usager)
)
...
, deleted_records as (
  ...
  from {{ this }} t
  inner join all_etat_usagers s
    on t.src_id = (s.src || '_' || s.etat_usagers_id)
  where t.is_current = true
