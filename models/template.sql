...
with all_type_activite as (
  select
    "ID" as type_activite_id,                   -- ######################## To be changed  IF in postgres id is miniscule put id if not THEN put "ID"
    code,                                       -- ######################## To be changed
    designation_latin as type_activite,         -- ######################## To be changed
    'AHS' as src,
  ...
  from {{ source('__raw_', 'ahs_wilaya') }}                                       -- ######################## To be changed
  ...
  select
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('__raw_', 'occ_wilaya') }}                                       -- ######################## To be changed
  ...
  select
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('__raw_', 'cz_wilaya') }}                                       -- ######################## To be changed
  ...
  select
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('__raw_', 'sahara_wilaya') }}                                       -- ######################## To be changed
),

-- Step 2: Prepare source data with natural key (src_id) and apply incremental filter
source_data as (
  select
    src || '_' || type_activite_id as src_id,
    type_activite_id,     -- ######################## To be changed  FROM HERE ON you put what is after AS 
    code,                 -- ######################## To be changed
    type_activite,        -- ######################## To be changed
    src,
    _ab_cdc_updated_at,
    _ab_cdc_deleted_at,
    row_number() over (partition by src || '_' || type_activite_id order by _ab_cdc_updated_at desc) as rn  -- ADDED
...
source_data_deduped as (  -- NEW CTE
  select
    src_id,
    type_activite_id,     -- ######################## To be changed
    code,                 -- ######################## To be changed
    type_activite,        -- ######################## To be changed
    src,
    _ab_cdc_updated_at,
...
, changed_records as (
  select
    s.src_id,
    s.type_activite_id,     -- ######################## To be changed
    s.code,                 -- ######################## To be changed
    s.type_activite,        -- ######################## To be changed
    s.src,
...
, expired_records as (
  select
    t.surrogate_key,
    t.src_id,
    t.type_activite_id,     -- ######################## To be changed
    t.code,                 -- ######################## To be changed
    t.type_activite,        -- ######################## To be changed
    t.src,
...
, deleted_records as (
  ...
  inner join all_type_activite s
    on t.src_id = (s.src || '_' || s.type_activite_id)
  where t.is_current = true
