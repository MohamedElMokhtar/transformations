...
-- Step 1: Collect all exercices from all RAW source tables
with all_exercices as (
  select
    id as exercices_id,
  annee as Année,
  "Type",
  trimestre as Période,
  Du,
  au,
    'AHS' as src,
  ...
  from {{ source('public', 'ahs_exercices') }}                                       -- ######################## To be changed
...
"Type",
  ...
  from {{ source('public', 'csm_exercices') }}                                       -- ######################## To be changed
  ...
  au,
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'occ_exercices') }}                                       -- ######################## To be changed
  ...
  au,
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'cz_exercices') }}                                       -- ######################## To be changed
...
source_data as (
  select
    ...
    _ab_cdc_updated_at,
    _ab_cdc_deleted_at,
    row_number() over (partition by src || '_' || exercices_id order by _ab_cdc_updated_at desc) as rn  -- ADDED
  from all_exercices
  {% if is_incremental() %}
...
, changed_records as (
  ...
  where
    ...
    (s.Année is distinct from t.Année)                              -- ######################## To be changed THIS PART you put only the data (don't put id)
    or (s."Type" is distinct from t."Type")
    or (s.Période is distinct from t.Période)
    or (s.Du is distinct from t.Du)
    or (s.au is distinct from t.au)         -- ######################## To be changed
)
...
, deleted_records as (
  ...
  from {{ this }} t
  inner join all_exercices s
    on t.src_id = (s.src || '_' || s.exercices_id)
  where t.is_current = true
