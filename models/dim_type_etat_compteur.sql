...
-- Step 1: Collect all E from all RAW source tables
with all_etat_compteur as (
  select
    id as etat_compteur_id,                   -- ######################## To be changed  IF in postgres id is miniscule put id if not THEN put "ID"                                  -- ######################## To be changed
    Designation_Latin as etat_compteur,         -- ######################## To be changed
    'AHS' as src,
...
source_data as (
  select
    ...
    etat_compteur_id,     -- ######################## To be changed  FROM HERE ON you put what is after AS 
    etat_compteur,        -- ######################## To be changed
    src,
    _ab_cdc_updated_at,
    _ab_cdc_deleted_at,
    row_number() over (partition by src || '_' || etat_compteur_id order by _ab_cdc_updated_at desc) as rn  -- ADDED
...
source_data_deduped as (  -- NEW CTE
  select
    src_id,
    etat_compteur_id,     -- ######################## To be changed
    etat_compteur,        -- ######################## To be changed
    src,
    _ab_cdc_updated_at,
    _ab_cdc_deleted_at
...
, changed_records as (
  select
    s.src_id,
    s.etat_compteur_id,     -- ######################## To be changed
    s.etat_compteur,        -- ######################## To be changed
    s.src,
...
, expired_records as (
  select
    t.surrogate_key,
    t.src_id,
    t.etat_compteur_id,     -- ######################## To be changed
    t.etat_compteur,        -- ######################## To be changed
    t.src,
