...
-- Step 1: Collect all type_paiement from all RAW source tables
with all_type_paiement as (
  select
    id as type_paie_id,
 designation_latin as type_paie,
    'AHS' as src,
  ...
  from {{ source('public', 'ahs_type_paiement') }}                                       -- ######################## To be changed
 ...
 designation_latin as type_paie,
  ...
  from {{ source('public', 'csm_type_paiement') }}                                       -- ######################## To be changed
 ...
 designation_latin as type_paie,
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'occ_type_paiement') }}                                       -- ######################## To be changed
 ...
 designation_latin as type_paie,
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'cz_type_paiement') }}                                       -- ######################## To be changed
 ...
 designation_latin as type_paie,
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'sahara_type_paiement') }}                                       -- ######################## To be changed
),

-- Step 2: Prepare source data with natural key (src_id) and apply incremental filter
source_data as (
  select
    src || '_' || type_paie_id as src_id,
    type_paie_id,
  type_paie,
    src,
    _ab_cdc_updated_at,
    _ab_cdc_deleted_at,
    row_number() over (partition by src || '_' || type_paie_id order by _ab_cdc_updated_at desc) as rn  -- ADDED
  from all_type_paiement
  {% if is_incremental() %}
...
, changed_records as (
  ...
  where
    ...
    (s.type_paie is distinct from t.type_paie)                              -- ######################## To be changed THIS PART you put only the data (don't put id)
            -- ######################## To be changed
)
...
, deleted_records as (
  ...
  from {{ this }} t
  inner join all_type_paiement s
    on t.src_id = (s.src || '_' || s.type_paie_id)
  where t.is_current = true
