...
with all_type_activite as (
  select
    "ID" as type_activite_id,
    code,
    designation_latin as type_activite,
    'AHS' as src,
  ...
  from {{ source('public', 'ahs_type_activite') }}
  ...
  from {{ source('public', 'csm_type_activite') }}
  ...
  select
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'occ_type_activite') }}
  ...
  select
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'cz_type_activite') }}
  ...
  select
    ...
    _ab_cdc_updated_at::timestamp as _ab_cdc_updated_at,
    _ab_cdc_deleted_at::timestamp as _ab_cdc_deleted_at
  from {{ source('public', 'sahara_type_activite') }}
),
...
, changed_records as (
  ...
  where
    ...
    (s.code is distinct from t.code)
    or (s.type_activite is distinct from t.type_activite)
)
